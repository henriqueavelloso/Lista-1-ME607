...
ggplot(dados, aes(x = Date, y = Close)) +
  geom_line(color = "blue") +  # Adiciona a linha do gráfico
...
```{r echo=TRUE}
# Calcular os retornos financeiros
retorno <- dados %>%
...
```{r}
# Gráfico da função de autocorrelação dos retornos
retorno %>%
  drop_na(Return) %>%  
  ACF(Return) %>%
  autoplot() +
...
```{r}
# Gráfico da função de autocorrelação parcial dos retornos
retorno %>%
  drop_na(Return) %>%  
  PACF(Return) %>%
  autoplot() +
